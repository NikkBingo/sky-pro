...
function finalManualExtract() {
    console.log('🔍 Final manual extraction of size charts...\n');
    
    // Read the Finnish translations file
    const finnishContent = fs.readFileSync('finnish-translations.csv', 'utf8');
    
    // Extract size charts by manually reading the specific content
    const sizeCharts = {};
...
function extractSizeChartFromContent(content, productCode) {
    ...
    if (sizeTableStart === -1) {
        console.log(`❌ No size table found for product ${productCode}`);
        return '';
    }
