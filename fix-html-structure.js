...
function fixHTMLStructure() {
    console.log('🔍 Fixing HTML structure by removing extra br tags...\n');
    
    // First, extract size charts from XML
    extractSizeChartsFromXML().then(sizeCharts => {
        // Then fix the content columns with clean HTML structure
        fixContentColumnsWithCleanHTML(sizeCharts);
    });
}
        ...
        try {
            ...
            for (let i = 0; i < products.length; i++) {
                ...
                if (sizeTableText && sizeTableText.trim()) {
                    ...
                    const cleanSizeTable = cleanHTMLStructure(decodedSizeTable);
...
function cleanHTMLStructure(html) {
    ...
    // Remove extra br tags between table elements
    ...
    // Remove br tags between table and its content
...
function fixContentColumnsWithCleanHTML(sizeCharts) {
    console.log('\n🔍 Fixing content columns with clean HTML structure...\n');
    ...
    writeCleanHTMLCSV(outRows, embeddedCount);
}
...
function writeCleanHTMLCSV(rows, embeddedCount) {
    console.log(`\n📝 Writing clean HTML CSV with ${embeddedCount} embedded size charts...`);
    
    const outputStream = fs.createWriteStream('KH-print_skypro_clean_html.csv', {
        encoding: 'utf8',
        flags: 'w'
    });
    ...
    outputStream.on('finish', () => {
        console.log('✅ Clean HTML CSV written successfully.');
        
        const stats = fs.statSync('KH-print_skypro_clean_html.csv');
        console.log(`📊 File size: ${stats.size} bytes`);
        
        // Check if size charts are present
        const fullContent = fs.readFileSync('KH-print_skypro_clean_html.csv', 'utf8');
        ...
        console.log(`\n🎉 Successfully created clean HTML CSV!`);
    });
}
