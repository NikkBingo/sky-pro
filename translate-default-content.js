...
async function translateDefaultContent() {
  try {
    ...
    return new Promise((resolve, reject) => {
      fs.createReadStream('KH-Print_Oy_translations_SkyPro.csv')
        ...
        .on('end', async () => {
          ...
          // Process each row and translate Default content to Finnish
          let updatedCount = 0;
          let matchedCount = 0;
          ...
          csvData.forEach(row => {
            ...
            let finnishDefaultContent = '';
            
            if (finnishProduct) {
              // Map fields based on the Field column
              switch (field) {
                case 'title':
                  translatedContent = finnishProduct.title || '';
                  finnishDefaultContent = finnishProduct.title || originalDefaultContent;
                  break;
                case 'body_html':
                  translatedContent = finnishProduct.description || '';
                  finnishDefaultContent = finnishProduct.description || originalDefaultContent;
                  break;
                case 'handle':
                  translatedContent = finnishProduct.code || '';
                  finnishDefaultContent = finnishProduct.code || originalDefaultContent;
                  break;
                case 'product_type':
                  // Map category to product type
                  const category = finnishProduct.categories?.category?.[0]?.name;
                  translatedContent = category || '';
                  finnishDefaultContent = category || originalDefaultContent;
                  break;
                default:
                  translatedContent = '';
                  finnishDefaultContent = originalDefaultContent;
              }
              
              if (translatedContent) {
                updatedCount++;
                console.log(`✅ Translated ${field} for product ${identification}: ${translatedContent.substring(0, 50)}...`);
              }
            } else {
              // Keep original content if no Finnish translation found
              finnishDefaultContent = originalDefaultContent;
            }
            
            // Create CSV row with proper escaping
            const escapedDefaultContent = `"${finnishDefaultContent.replace(/"/g, '""')}"`;
            const escapedTranslatedContent = `"${translatedContent.replace(/"/g, '""')}"`;
          ...
          // Write updated CSV with Finnish translations in Default content
          fs.writeFileSync('KH-Print_Oy_translations_SkyPro_translated.csv', updatedRows.join('\n'));
          
          console.log('✅ CSV with Finnish translations in Default content created successfully!');
          console.log(`📊 Summary:`);
          ...
          console.log(`   - Output file: KH-Print_Oy_translations_SkyPro_translated.csv`);
