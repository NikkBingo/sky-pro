...
async function importApparelFinnishCorrected() {
  try {
    ...
    // Define Finnish categories that correspond to "Apparel"
    const apparelRelatedCategories = [
      'T-paidat',
      'Pikeet', 
      ...
      'Kauneus- ja hoiva-alan vaatteet',
      'Beauty and Healthcare',
      'Lyhythihaiset',
      'Pitkähihaiset',
      'Pitkähihaiset kauluspaidat',
      'Lyhythihaiset kauluspaidat',
      'O-aukkoiset',
      'V-aukkoiset',
      'Vetoketjulliset',
      'Vetoketjuttomat',
    ...
    // Filter for Apparel-related category products
    const apparelProducts = productArray.filter(product => {
      if (product.categories?.category) {
        ...
        return categories.some(cat => 
          cat.name && apparelRelatedCategories.includes(cat.name)
        );
      }
      return false;
    });
    
    console.log(`👕 Found ${apparelProducts.length} Apparel-related products in Finnish feed`);
    ...
    return new Promise((resolve, reject) => {
      fs.createReadStream('KH-Print_Oy_translations_SkyPro.csv')
        ...
        .on('end', async () => {
          ...
          // Write updated CSV
          fs.writeFileSync('KH-Print_Oy_translations_SkyPro_apparel_corrected.csv', updatedRows.join('\n'));
          ...
          console.log(`   - Apparel-related products in Finnish feed: ${apparelProducts.length}`);
          console.log(`   - Apparel products matched: ${matchedCount}`);
          console.log(`   - Finnish translations added: ${updatedCount}`);
          console.log(`   - Output file: KH-Print_Oy_translations_SkyPro_apparel_corrected.csv`);
          console.log(`\n📝 Apparel Import Strategy:`);
          console.log(`   1. Map English "Apparel" to Finnish clothing categories`);
...
// Run the corrected Apparel import
importApparelFinnishCorrected(); 