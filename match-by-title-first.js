...
async function matchByTitleFirst() {
  try {
    ...
    console.log(`🗺️  Created maps: ${finnishProductMapByCode.size} by code, ${finnishProductMapByTitle.size} by title`);
    ...
    return new Promise((resolve, reject) => {
      fs.createReadStream('KH-Print_Oy_translations_SkyPro.csv')
        ...
        .on('end', async () => {
          ...
          // Group rows by product ID to find titles first
          const rowsByProduct = {};
          csvData.forEach(row => {
          ...
          // Find titles for each product first
          const productTitles = {};
          Object.keys(rowsByProduct).forEach(productId => {
            const titleRow = rowsByProduct[productId].find(row => row.Field === 'title');
            if (titleRow) {
              productTitles[productId] = titleRow['Default content'];
            }
          });
          
          console.log(`📝 Found ${Object.keys(productTitles).length} product titles`);
          
          // Process each row with title-based matching
          let updatedCount = 0;
          let matchedCount = 0;
          ...
          csvData.forEach(row => {
            ...
            // Get the product title for this product
            const productTitle = productTitles[row.Identification];
          ...
          // Write updated CSV
          fs.writeFileSync('KH-Print_Oy_translations_SkyPro_title_matched.csv', updatedRows.join('\n'));
          
          console.log('✅ CSV with title-first matching created successfully!');
          console.log(`📊 Summary:`);
          ...
          console.log(`   - Output file: KH-Print_Oy_translations_SkyPro_title_matched.csv`);
          console.log(`\n📝 Matching Strategy:`);
          console.log(`   1. Match by product title first`);
          console.log(`   2. Fallback to code matching`);
          console.log(`   3. Keep Default content as English`);
          console.log(`   4. Add Finnish to Translated content`);
